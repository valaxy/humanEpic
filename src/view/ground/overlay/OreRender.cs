...
public partial class OreRender : OverlayRender
{
    public override List<ComponentDefinition> GetComponentDefs(float scale)
    {
        ...
        mesh.Rings = 1;
        return new List<ComponentDefinition> { new ComponentDefinition("crystal", mesh, new Color(0.6f, 0.2f, 0.8f)) };
    }
    ...
    public override List<InstanceData> GetCellInstances(int x, int y, Ground ground, float scale)
    {
        ...
        res.Add(new InstanceData {
            ...
            LocalTransform = t1,
            VisualOffset = new Vector3(0, 0.2f * scale, 0)
        });
        ...
        res.Add(new InstanceData {
            ComponentName = "crystal",
            LocalTransform = t2.Scaled(new Vector3(0.7f * scale, 0.7f * scale, 0.7f * scale)),
            VisualOffset = new Vector3(0.2f * scale, 0.1f * scale, 0.1f * scale)
        });
        ...
        res.Add(new InstanceData {
            ComponentName = "crystal",
            LocalTransform = t3.Scaled(new Vector3(0.6f * scale, 0.6f * scale, 0.6f * scale)),
            VisualOffset = new Vector3(-0.15f * scale, 0.1f * scale, -0.2f * scale)
        });
